...
class SettingController extends Controller
{
    ...
    public function deleteMySetting(Request $request)
    {
        ...
        try {
            ...
            return redirect('/rc-setting/userpage/?account_id='. Auth::user()->account_id)
    ...
    public function editMySetting($settingId)
    {
        ...
        try {
            ...
            return view('contents.edit_setting')
            ->with('targeEdittSettingId', $targeEdittSettingId);
    ...
    public function updateMySetting(Request $request)
    {
        ...
        if ($validator->fails()) {
            return redirect('/rc-setting/edit/setting/mysetting')
            ->withErrors($validator)
            ->withInput();
        }
        $accountUuid = $request->targetAccountId;
        ...
        try {
            ...
            $updateMySetting = MySetting::updateMySetting($updateSettingtData);

            DB::commit();

            Log::debug("message",[$updateMySetting]);

            return redirect('/rc-setting/top')
            ->with('succsess_message','okまる');
        } catch (\Throwable $th) {
            Log::error("セッティングの更新で例外処理発生",[$th]);
        }
    }
}
